DrawApp = (function () {
  ...
  const savePosition = (shape) => {
    if (shape) {
      ...
      //  90 / 180: w == h
      //  0 / 270: w == w, h == h
      let width = shape.attrs.width;
      let height = shape.attrs.height;
      let rotationWidth, rotationHeight;
      ...
      shape.temporary = {
        theta: theta,
        rotPoint: {x: group.x(), y: group.y()},
        groupX: shape.attrs.startX,
        groupY: shape.attrs.startY,
        width: width,
        height: height,
        rotatedWidth: rotationWidth,
        rotatedHeight: rotationHeight,
      };
    }
  };
  ...
  const setToTemporaryPosition = (shape) => {
    if (shape && shape.temporary) {
      if (shape.temporary.theta === 0) {
        ...
        if (theta === 90 || theta === 270){
          width = shape.temporary.rotatedWidth;
          height = shape.temporary.rotatedHeight;
        }
