Resize = (function() {
  ...
  let start = {x: 0, y: 0};
  let activeAnchor, previewBox, currentShape;
  ...
  const resizeEnd = () => {
    ...
    const oldPosition = (currentShape.attrs.temporary ? Object.assign(currentShape.attrs.temporary) : undefined);
    try{
      if (currentShape.lineType()) {
      ...
      console.log('error', e);
      if (oldPosition) {
        currentShape.attrs.temporary = Object.assign(oldPosition);
      }
      DrawApp.setToTemporaryPosition(currentShape);
    }
  }

  const setNewTemporary = () => {
    const ellipse = currentShape.ellipseType();
    let newX, newY;
    if (ellipse) {
      newX = previewBox.width()/2;
      newY = previewBox.height()/2;
    }
    currentShape.temporary = {
      theta: currentShape.attrs.rotationAngle,
      groupX: previewBox.x(),
      groupY: previewBox.y(),
