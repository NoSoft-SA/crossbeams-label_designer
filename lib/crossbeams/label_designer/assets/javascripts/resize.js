const Resize = (function () {
  ...
  const setCurrentValues = (anchor) => {
    if (anchor) {
      try {
        ...
        console.error('anchor does not have a group');
  ...
  const getOppositeAnchor = () => {
    if (activeAnchor) {
      ...
      console.error('no anchor was given');
    }
    return null;
  };
    ...
    if (currentShape) {
      if (currentShape.lineType()) {
        ...
        switch (activeAnchor.getName()) {
          case 'startPoint':
            newX = ((drawEnv.shifted === true) ? currentShape.drawnShape.points()[2] : anchorX);
            newY = ((drawEnv.controlled === true) ? currentShape.drawnShape.points()[3] : anchorY);
            ...
            break;
          case 'endPoint':
            newX = ((drawEnv.shifted === true) ? currentShape.drawnShape.points()[0] : anchorX);
            newY = ((drawEnv.controlled === true) ? currentShape.drawnShape.points()[1] : anchorY);
            endPoint.setX(newX);
            endPoint.setY(newY);
        ...
        // Positioner.updateCurrentCoords(undefined);
        Positioner.updateCurrentCoords();
  ...
  const before = (event, anchor) => {
    if (anchor) {
      ...
      console.error('no anchor for resizing');
    }
  };
