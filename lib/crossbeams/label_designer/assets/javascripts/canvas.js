Canvas = (function () {
  ...
  const reinitCanvas = (inStage) => {
    currentStage = inStage;
    currentImageLayer = inStage.getChildren(node => node.attrs.name === 'image')[0];
    // currentImageLayer = inStage.getChildren(function (node) {
    //   return node.attrs.name === 'image';
    // })[0];
    currentVariableLayer = inStage.getChildren(node => node.attrs.name === 'variable')[0];
    // currentVariableLayer = inStage.getChildren(function (node) {
  ...
  const setScale = (scale) => {
    if (currentScale != scale) { // Is != REQUIRED here? or should it be !==
      currentScale = scale;
