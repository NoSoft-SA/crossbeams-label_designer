VariableSettings = (function () {
  ...
  const whiteOnBlack = document.querySelector('#white_on_black');
  ...
  const toggleBarcodeOptions = (checked) => {
    if (checked) {
      whiteOnBlack.checked = false;
      whiteOnBlack.disabled = true;
      barcodeOptions.style.display = 'block';
    } else {
      whiteOnBlack.disabled = false;
  ...
  const save = (shape) => {
    if (shape.variableBoxType()) {
      ...
      const currentValues = {
        ...
        barcodeSymbology,
        whiteOnBlack: String(whiteOnBlack.checked),
      };
  ...
  const openDialog = (shape) => {
    if (shape) {
      ...
      if (info) {
        ...
        whiteOnBlack.checked = info.whiteOnBlack && info.whiteOnBlack === 'true';
  ...
  // When white-on-black is toggled, change the
  // border and test colour.
  const varColourChange = (value) => {
    const thisVar = MyLabel.selectedShape();
    if (value) {
      thisVar.textBox.fill('#ca48bc');
      thisVar.outerBox.stroke('#ca48bc');
    } else {
      thisVar.textBox.fill('black');
      thisVar.outerBox.stroke('#188FA7');
    }
    thisVar.outerBox.draw();
    thisVar.textBox.draw();
  };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // });
    whiteOnBlack.addEventListener('change', (event) => {
      varColourChange(event.target.checked);
    });
