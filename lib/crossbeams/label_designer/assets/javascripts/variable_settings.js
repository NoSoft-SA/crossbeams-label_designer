VariableSettings = (function() {
  ...
  $('#variable-info-dialog-form').on('close.popupwindow', function() {
    ...
    if (shape.variableSettings.variableType === '0') {
  ...
  const save = (shape) => {
    if (shape.variableBoxType()) {
      ...
      shape.variableSettings = {
        ...
        fontFamily: shape.textSettings.fontFamily,
        bold: shape.textSettings.bold,
        italic: shape.textSettings.italic,
  ...
  const update = (attribute) => {
    const shape = MyLabel.selectedShape();
    if (shape.variableBoxType()) {
      const settings = shape.variableSettings;
      switch (attribute) {
        case 'position':
          const pos = Library.actualPosition(shape.group);
        ...
        case 'alignment':
          ...
          break;
        case 'font-family':
          settings.fontFamily = shape.textSettings.fontFamily;
          break;
        case 'bold':
          settings.bold = shape.textSettings.bold;
          break;
        case 'italic':
          settings.italic = shape.textSettings.italic;
          break;
        case 'underline':
          settings.underline = (shape.textBox.textDecoration() === 'underline');
          break;
      }
    }
  }
