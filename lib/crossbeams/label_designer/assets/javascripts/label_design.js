(function () {
  ...
  function getContainerOffset(event) {
  ...
  function getMouseCoordsRelativeToPage(event) {
  ...
  function updateStartCoords(event) {
    [drawEnv.startX, drawEnv.startY] = getCoords(event);
  }
  function updateCurrentCoords(event) {
    [drawEnv.currentX, drawEnv.currentY] = getCoords(event);
  }
  ...
  document.addEventListener('mousemove', event => {
    const text = getCoords(event).map(item => (item < 0 ? 0 : item)).join(', ');
