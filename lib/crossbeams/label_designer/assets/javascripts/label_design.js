const Ld = (function Ld() {
  ...
  function populateForm(shape) {
    if (shape) {
      ...
      // This function gets the closest value in an array of integers.
      const getClosestPixelSize = (arr, px) => {
        let lo;
        let hi;
        arr.forEach((elem) => {
          if (elem <= px && (lo === undefined || lo < elem)) lo = elem;
          if (elem >= px && (hi === undefined || hi > elem)) hi = elem;
        });
        if (lo === hi) return lo;
        if (lo === undefined) return hi;
        if (hi === undefined) return lo;
        if (px - lo > hi - px) return hi;
        return lo;
      };

      // The reason we need to look for the closest pixel-match is because the font-table
      // of pixels -> points might change and then we might not be able to find a matching
      // pixel value in the list of font sizes when editing an existing label.
      let px = shape.textBox.fontSize();
      if (fontSizes[px] === undefined) {
        const pixelSet = Object.keys(fontSizes).map(k => Number(k)).sort((a, b) => a - b);
        px = getClosestPixelSize(pixelSet, px);
        // Update the shape's font size to the new pixel value.
        shape.textBox.fontSize(px);
      }
      const fontSizeOption = document.querySelector(`#font-size option[value='${px}']`);
  ...
  // Make all font family choices available for text boxes.
  // For variables, only Arial, Courier and Times are available.
  function setAllowedFontOptions(boxType) {
    const fontFamilyOption = document.querySelector('#font-family');
    Array.from(fontFamilyOption.children).forEach((li) => { li.disabled = false; });
    if (boxType === 'variable') {
      Array.from(fontFamilyOption.children).forEach((li) => {
        if (li.value === 'Lato Light') {
          li.disabled = true;
        }
      });
    }
  ...
  function toggleOptions() {
    ...
    const textPopups = document.querySelectorAll('.text-popup');
    textPopups.forEach((tp) => { tp.dataset.disabled = true; });
    const textInputBox = document.querySelector('#textinput');
    textInputBox.blur();
    ...
    strokeWidthSelector.disabled = true;
    strokeWidthSelector.dataset.disabled = true;
    ...
    if (shape) {
      ...
      switch (shape.attrs.name) {
        case 'VariableBox':
          ...
          textPopups.forEach((tp) => { tp.dataset.disabled = false; });
          textInputBox.placeholder = 'Demo text';
          varSettingsButton.removeAttribute('style');
          setAllowedFontOptions('variable');
          textInputBox.innerHTML = shape.textBox.text();
          TextSettings.focusTextbox();
          break;
        case 'TextBox':
          ...
          textPopups.forEach((tp) => { tp.dataset.disabled = false; });
          setAllowedFontOptions('text');
          textInputBox.innerHTML = shape.textBox.text();
          TextSettings.focusTextbox();
          break;
        default:
          if (shape.drawnShape) {
            const strokeWidthOption = document.querySelector(`#stroke-width option[value='${shape.drawnShape.strokeWidth()}']`);
            document.querySelector('#stroke-width').selectedIndex = (strokeWidthOption !== null) ? strokeWidthOption.index : 0;
            strokeWidthSelector.disabled = false;
            strokeWidthSelector.dataset.disabled = false;
          } else {
            document.querySelector('#stroke-width').selectedIndex = 0;
          }
          break;
  ...
  const Shape = function Shape() {
    return {
      ...
      reinitialise(konvaGroup, shape) {
        ...
        Object.keys(this.defaultAttrs()).forEach((prop) => {
          if (!(prop in newAttrs)) {
      ...
      defaultAttrs(prop = undefined) {
        const attrs = {
          ...
          width: 0,
          height: 0,
          fontSizePx: fontDefaultPx,
          fontSizePt: fontDefaultPt,
          alignment: 'left',
          fontFamily: 'Arial',
          bold: false,
          italic: false,
      ...
      addAnchor(group, x, y, name) {
        ...
        anchor.on('dragmove', () => {
          Resize.resizeStart(); // Bombs if event passed in...
          if (layer) {
            layer.draw();
          }
        });
        anchor.on('mousedown touchstart', function mouseTouch(event) {
          group.setDraggable(false);
          this.moveToTop();
          Resize.before(event, this);
        });
        anchor.on('dragend', () => {
          Resize.resizeEnd();
  ...
  const Label = function Label() {
    let nextShapeId = 0;
    return {
      openLabel(thisLabelConfig = undefined) {
        if (thisLabelConfig) {
          const { width, height } = MyLabelSize;
          if (thisLabelConfig.labelState === 'preview') {
            // return preview only
          } else {
            // Load initial label
            Canvas.resetCanvas(width, height);

            if (thisLabelConfig.labelState === 'new') {
              this.initialise(width, height, undefined);
            } else {
              this.initialise(undefined, undefined, thisLabelConfig.labelJSON);
            }
            Canvas.drawCanvas();
          }
        } else {
          throw new Error('ERROR: Invalid label config.');
        }
      },
      ...
      loadFromJson(labelJSON) {
        ...
        nextShapeId = this.shapes.length + 1;
      ...
      // For debugging purposes: download the state of the label.
      debugDump(filename) {
        const labelState = {
          label: this,
          undoState: UndoEngine.debugUndoStack(),
          images: 'to come if required',
        };
        const blob = new Blob([JSON.stringify(labelState)], { type: 'application/json;charset=utf-8;' });
        const encodedUri = URL.createObjectURL(blob);
        // window.open(encodedUri); //... This will work in Firefox, below only in Chrome...
        const link = document.createElement('a');
        link.setAttribute('href', encodedUri);
        link.setAttribute('download', `${filename}.json`); // From Chrome 35.0.1916.114, the filename is no longer honoured.
        link.click();
      },
      toImageDataURL() {
      ...
      generateXMLFile() {
        ...
        for (const [i, shape] of MyLabel.variables.entries()) {
          ...
          if (info) {
            if (info.staticValue !== null && info.staticValue !== undefined) {
              const variableString = `<static_barcode><id>${info.variableId
              }</id><static_value>${info.staticValue
              }</static_value><variable_type>${info.variableType
              ...
              }</fontsize_px><white_on_black>${info.whiteOnBlack || 'false'
              }</white_on_black><fontsize_pt>${info.fontSizePt
              ...
              }</height><white_on_black>${info.whiteOnBlack || 'false'
              }</white_on_black><fontsize_px>${info.fontSizePx
  ...
  ready(() => {
    ...
    // Keep the top ruler in sync with vertical scrolling of the drawing area.
    document.querySelector('div.designer-container').addEventListener('scroll', () => {
      const designContainer = document.querySelector('div.designer-container');
      const topRuler = document.querySelector('div.top-ruler');
      topRuler.style.left = `-${designContainer.scrollLeft}px`;
    });
  });
