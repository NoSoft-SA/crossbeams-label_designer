    ...
    return {
      ...
      loadFromJson(labelJSON) {
        ...
        newLabel.shapes.forEach((shape) => {
          ...
          console.log(Canvas.variableLayer())
          console.log(Canvas.imageLayer())
          const newShape = new Shape();
          let newGroup;
          if (shape.attrs.name === 'VariableBox') {
            newGroup = Canvas.variableLayer().getChildren(function (node) {
              console.log('variables')
              console.log('node id', node.id())
              console.log('shape id', shape.shapeId)
              return node.id() === shape.shapeId;
            })[0];
            console.log(newGroup)
          } else {
            newGroup = Canvas.imageLayer().getChildren(function (node) {
              console.log('images')
              console.log('node id', node.id())
              console.log('shape id', shape.shapeId)
              return node.id() === shape.shapeId;
          ...
          console.log('Inside shapes.each')
      ...
      removeShape(shapeToRemove) {
        ...
        this.variables.splice(this.shapes.indexOf(shapeToRemove), 1);
        Canvas.drawCanvas();
      },
      getShapeByElem(elem) {
      ...
      exportToJSON() {
        // TODO: Validate shapes before save
        Canvas.setScale(1);
