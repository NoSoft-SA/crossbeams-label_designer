    ...
    return {
      ...
      rotate() {
        if (this.variableBoxType()) {
          ...
          // DrawApp.savePosition(this)
          // DrawApp.inplace90DegreeTurn(this)
        } else {
          this.group.rotate(90);
        }
        Canvas.drawCanvas();
      ...
      setBoxDimensions(width, height) {
        if (!this.lineType()) {
          ...
          let newHeight = (this.attrs.squareConstraint) ? newWidth : Math.abs(height);
          newHeight = (this.attrs.keepAspectRatio) ? Math.abs(this.attrs.aspectRatio*newWidth) : Math.abs(height);
      ...
      resetRotationPoint(activeAnchor = undefined) {
        let stableAnchor, stableAnchorName, stablePointAnchorName;
        if (this.lineType()) {
      ...
      addAnchor(group, x, y, name) {
        ...
        const anchor = new Konva.Circle({
          ...
          radius: 4,
          name,
          draggable: (name === 'startPoint') ? false : true,
          dragOnTop: false,
        });
