(function () {
  ...
  const drawEnv = {
    ...
    offset: findAbsolutePos($('#paper')[0]),
    drawing: 'none'
  };
  ...
  // Set eventlisteners for canvas in drawApp, but maybe call these functions from the drawApp
  ...
  $(document).on('mousewheel', event => {
    // drawEnv.offset = findRelativePos(Canvas.container())
    // console.log('offset updated', drawEnv.offset);
  });
  ...
  // function findRelativePos(obj) {
    // TODO: all positions for the Label designer needs to be confined strictly to the position of the container
    // Calculate offset by getting absolute position of container,
    // absolute offset of canvas (div paper)
    // scrollLeft and scrollTop of container
  //   let currentLeft = 0,
  //       currentTop = 0;
  //   if (obj.offsetParent) {
  //     do {
  ...
  //       currentTop += obj.offsetTop;
  //     } while (obj = obj.offsetParent);
  //     currentLeft += (document.querySelector('.designer-container').scrollLeft - document.body.clientLeft);
  //     currentTop += (document.querySelector('.designer-container').scrollTop - document.body.clientTop);
  //     return [currentLeft, currentTop];
  //   }
  //   return false;
  // }
