...
class ExternalLabel extends Array {
  ...
  loadFromJson(labelJSON) {
    ...
    // newLabel.forEach(function(shape) {

    //   let imageParams;
    //   if (shape.shapeType === 'ImageShape') {
    //     const img = new Image;
    //     img.onload = function () {
    //       Canvas.drawCanvas();
    //     };
    ...
    //   const newShape = ExternalShape.create(shape.shapeType.split('Shape')[0], (imageParams || undefined), myLabel)
    ...
    //   myLabel.push(newShape);
    // });

    // this.selectShape(this[this.length - 1]);
    this.stage.draw();
  }
  setAsCurrent() {
    this.currentLabel = true;
  ...
  newShape(type, imageParams = undefined) {
    const shape = ExternalShape.create(type, imageParams, this);
  ...
  restrictionSet() {
    this.variableSet().map(shape => Library.actualPosition(shape.group));
  }
  widthRestriction() {
  ...
  buildXMLHeader() {
    return `<?xml version="1.0" encoding="UTF-8"?><label_schema>` +
      `<image_description><image_filename>${ 'This needs to move to the properties file'
