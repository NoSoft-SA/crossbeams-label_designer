(function() {
  ...
  function getImage(group){
    var image = group.getChildren(function(node){
  ...
  var Shape = function Shape(){
    ...
    return{
      ...
      initialise: function(name, imageObj, width, height){
        this.name = name
        this.type = ((['VariableBox', 'TextBox', 'Image'].indexOf(name) > -1) ? 'Group' : name);

        this.primary = this[('init' + name)](imageObj, width, height);
        drawEnv.currentElement = this;

        // ['VariableBox', 'TextBox', 'Image', 'Rect', 'Line', 'Ellipse']
        // primary will then always be the group
        // rect shouldn't have a box
        // Line should only have 2 anchor points
        // or resizing and anchor points should vary for different shapes
        // for shapes a redraw makes more sense than resizing

        if ('Group' === this.type){
          this.drawn = getRectangle(this.primary);
          if ('Image' === this.name){
            this.image = getImage(this.primary);
          } else if ('VariableBox' === this.name || 'TextBox' === this.name){
            this.textBox = getTextBox(this.primary);

          } else if ('Line' === this.name){

          } else if ('Rect' === this.name){
          } else if ('Ellipse' === this.name){
          } else {
            this.textBox = getTextBox(this.primary);
          }
        } else{
        ...
        if (this.name === 'Image'){
          drawEnv.imageLayer.add(this.primary);
