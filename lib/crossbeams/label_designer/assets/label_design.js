(function () {
  ...
  const LabelDesigner = function LabelDesigner() {
    return {
      addUndoEvent() {
        UndoEngine.addCommand({
          action: 'move',
          current: myLabel.exportToJSON()['label'],
          previous: previousLabel['label'],
          executeUndo() {
            myLabel.initialise(null, null, 'test', 1, this.previous);
          },
          executeRedo() {
            myLabel.initialise(null, null, 'test', 1, this.current);
          }
        });
        return true;
      }
...
// myLabelDesigner = new LabelDesigner();
//     myLabel = new Label();

//     // NEW LABEL:
//     // needs a name
//     // TODO: Save as name for image
//     myLabel.initialise(500, 300, 'test');
//     // TO LOAD A LABEL:
//     // myLabel.initialise(null, null, 'test', 1, testJSON);
//     //  does not use the image - replaces it if label is reopened
//     //  XML is also regenerated
//     //  So technically it just needs the LabelJSON here to reinit
//     // TO SAVE A LABEL:
//     // myLabel.exportToJSON(),
  ...
  // const loadLabel = <%= @label_variables %>
  ...
  console.log('loadlabel',labelConfig);
  console.log('size',myLabelSize);
  ...
  $(document).on('keydown', (event) => {
    const index = [46, 37, 39, 38, 40].indexOf(event.keyCode);
    if (index > -1 && (event.target.nodeName !== 'TEXTAREA')) {
  ...
  function doResizeCanvas(width, height, originalWidth = null, originalHeight = null) {
    if (myLabel && (width >= myLabel.minLabelWidth)) {
    ...
    if (myLabel && (height >= myLabel.minLabelHeight)) {
  ...
  function activateTool(toolHandle) {
    ...
    if ($(toolHandle).hasClass('selected')) {
      // $(toolHandle).removeClass('selected');
  ...
  const Shape = function Shape() {
    ...
    return {
      ...
      remove(force = false) {
        ...
        if (executed) { myLabel.afterAction(); }
      },
      rotate() {
        myLabel.beforeAction();
  ...
  const Label = function Label() {
    let nextShapeId = 0;
    return {
      // initialise(width = 700, height = 500, name = null, id = null, labelJSON = null) {
      initialise(width = undefined, height = undefined, name = undefined, id = undefined, labelJSON = undefined, labelConfig = undefined) {
        if (labelConfig){
          this.id = ((labelConfig.labelState === 'copy') ? undefined : (labelConfig.id || id));
          this.name = ((labelConfig.labelState === 'copy') ? (name || `${labelConfig.labelName}_copy`) : (labelConfig.labelName || name));
          if (labelConfig.labelState === 'preview') {
            // return preview only
          } else {
            // Load initial label
            console.log(myLabelSize.width);
            console.log(myLabelSize.height);
            cleanStage(myLabelSize.width, myLabelSize.height);
            console.log('JSON one',labelJSON);
            labelJSON = ((labelConfig.labelState === 'new') ? undefined : labelConfig.labelJSON);

            console.log('state',labelConfig.labelState);
            console.log('JSON two',labelJSON);
      ...
      beforeAction() {
        previousLabel = this.exportToJSON();
      },
      afterAction() {
        myLabelDesigner.addUndoEvent();
        updateUndoButtons();
      },
      setCanvasMinimums() {
      ...
      generateXMLFile() {
        let XMLString = `<?xml version="1.0" encoding="UTF-8"?><label_schema><image_description>
        <image_filename>${this.imageName
        }</image_filename><width>${this.labelWidth
        }</width><height>${this.labelHeight
        }</height><variable_count>${drawEnv.variableLayer.getChildren().length
  ...
  ready(() => {
    ...
    myLabel.initialise(drawEnv.initWidth, drawEnv.initHeight, `Copy of ${labelConfig.labelName}`, undefined, undefined, labelConfig );
