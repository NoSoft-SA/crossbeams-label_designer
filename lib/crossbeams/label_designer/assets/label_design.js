(document.onload = function() {
  ...
  div_paper.onmousedown = function(event) {
    ...
    if (drawingLine === true){
      drawLine(event);
    } else if (drawingRectangle === true){
      drawRectangle(event);
    } else if (drawingEllipse === true){
      drawEllipse(event);
    } else if (drawingTextBox === true){
      drawTextBox(event);
    } else if (drawingVariableBox === true){
...
// THESE 3 SHOULD BE IN A CASE
  ...
  // I can either add in ellipse OR call this ellipse and make shift force ellipse
  $('button[name="ellipse"]').click(function(){
    ...
    // deselect others
    // deselect_others($(this).attr('name'))
