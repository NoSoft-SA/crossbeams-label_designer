...
// $('button[name=line]')
  ...
  $('.btn-download-image').click(function(){
    ...
    var href = prepare_image();
    $('#btn-download-image').attr('href', href);
    $('#btn-download-image').trigger('click');
  });
  ...
  function prepare_image(){
    ...
    // var context = image_layer.getContext("2d");
    // console.log(image_layer);
    // console.log(context);
    // console.log(context['canvas']);
    // var dataURL = context[0].toDataURL('image/png');
    return dataURL;
  }
    // button.href = dataURL;

    // var image = new Image;
    // image.src = "label_image.svg";
    // image.onload = function() {
    //   context.drawImage(image, 0, 0);

    //   var a = document.createElement("a");
    //   a.download = "label_image.png";
    //   a.href = canvas.toDataURL("image/png");
    //   a.click();
    // };

// <button type="button" class='btn btn-default btn-md'><a href="#" id="btn-download-image"><i class="fa fa-download" aria-hidden="true"></i> Download IMG</a></button>
// <button type="button" class='btn btn-default btn-md'><a href="#" id="btn-download-xml"><i class="fa fa-download" aria-hidden="true"></i> Download XML</a></button>
...
// Draw the shape based on the current and start coordinates
  ...
  function drawVariableBox(event) {
    ...
    currentVariableRectangle = rect;
    currentElement.rectangle = currentVariableRectangle;

    div_paper.onmousemove = doDrawVariableBox;
  }

  function doDrawVariableBox(event) {
    if (currentElement && currentVariableRectangle) {
      updateCurrentCoords(event);
      // maybe split this out of here
      var width = currentX - startX;
      var height = currentY - startY;
      ...
      // function doDrawRectangle(event) {
      currentVariableRectangle.x( (width < 0) ? currentX : startX );
      currentVariableRectangle.y( (height < 0) ? currentY : startY );
      currentVariableRectangle.width( Math.abs(width) );
      currentVariableRectangle.height( Math.abs(height) );

      // currentVariableRectangle.draggable(true);
      variable_layer.add(currentElement);
      variable_layer.add(currentVariableRectangle);
      stage.draw();
    }
  }
  ...
  function shape(type){
    ...
    this.type = type;
    this.types = ['line', 'circle', 'rectangle', 'text_box', 'variable'];
