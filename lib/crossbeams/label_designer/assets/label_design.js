(function () {
  ...
  // const loadLabel = <%= @label_sizes %>
  // const loadLabel = <%= @label_variables %>
  const loadLabel = <%= @label_config %>
  ...
  // #################### CANVAS FUNCTIONS ####################################################
  cleanStage(drawEnv.initWidth, drawEnv.initHeight);

  $('.resizable').resizable({
    resize(event, ui) {
      const height = $('.resizable').height();
  ...
  $('#canvas_width').on('change', function () {
    const height = $('.resizable').height();
    const originalWidth = $('.resizable').width();
  ...
  $('#canvas_height').on('change', function () {
    ...
    const originalHeight = $('.resizable').height();
    const width = $('.resizable').width();
    ...
    return {
      ...
      reinitialise(konvaGroup, shape) {
        ...
        if (shape.name === 'Image') {
          ...
          this.addAnchors();
          drawEnv.stage.draw();
        } else if (shape.name === 'VariableBox' || shape.name === 'TextBox') {
          this.textBox = getChildOfType('Text', this.group);
        } else {
      ...
      remove(force = false) {
        var executed = false;
        if (this.name === 'VariableBox' && !force) {
        ...
        if (this.name === 'VariableBox') { myLabel.setCanvasMinimums(); }
      ...
      preventOverlap() {
        if ((this.name === 'VariableBox') && overlap(this.outerBox)) {
  ...
  const Label = function Label() {
    ...
    return {
      initialise(width = 700, height = 500, name = null, id = null, labelJSON = null) {
        ...
        if (name) {
          this.name = name;
          this.saveName = name.toLowerCase().replace(' ', '_');
          this.imageName = `${this.saveName}.png`;
        }
        if (labelJSON) {
          ...
          this.stage = cleanStage(width, height);
