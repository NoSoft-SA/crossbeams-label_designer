(document.onload = function() {
  ...
  div_paper.onmouseup = function(event) {
    ...
    if (drawing !== 'none'){
      $('button[name=' + drawing + ']').removeClass('selected');
      toggleOptions();
    }
    resetToolbar();
  };
...
// #################### SHAPE OPTIONS #######################################################
  $("input[name='textbox']").on('input', function(event){
    selectedElement.textBox.text($(this).val());
    stage.draw();
  });

  $('#font-size').on('change', function(){
    selectedElement.textBox.fontSize($(this).val());
    stage.draw();
  });

  $('#stroke-width').on('change', function(){
    selectedElement.drawn.strokeWidth($(this).val());
    stage.draw();
  });

  $('#font-family').on('change', function(){
    selectedElement.textBox.fontFamily($(this).val());
    stage.draw();
  });
  ...
  $('button.underline').on('click', function(){
    if ($(this).hasClass('selected')){
      ...
      selectedElement.textBox.textDecoration('');
    } else {
      $(this).addClass('selected');
      selectedElement.textBox.textDecoration('underline');
    }
    stage.draw();
  });
  ...
  $('button.bold, button.italic').on('click', function(){
    if ($(this).hasClass('selected')){
      $(this).removeClass('selected');
      selectedElement.textBox.fontStyle('normal');
    } else {
      $('button.bold').removeClass('selected');
      $('button.italic').removeClass('selected');
      $(this).addClass('selected');
      selectedElement.textBox.fontStyle($(this).attr('name'));
    }
    stage.draw();
  });
  ...
  $( 'button[name="Text"], \
      ...
      button[name="Group"]').click(function(){
        drawing = ((activateTool(this) === true) ? $(this).attr('name') : 'none');
  });
  ...
  $('button[name="pointer"]').click(function(){
    ...
    $('button[name="Text"').removeClass('selected');
    $('button[name="Group"').removeClass('selected');
  });


  var drawFunctions = {
    initLine: function initLine(event) {
    ...
    initRectangle: function initRectangle(event) {
    ...
    initEllipse: function initEllipse(event) {
    ...
    initText: function initText(event) {
    ...
    initGroup: function initGroup(event) {
      var text = new Konva.Text({
        x: startX,
        y: startY,
        text: 'Insert text...',
    ...
    drawShape: function drawShape(event) {
      if (currentElement) {
        ...
        var object = currentElement.primary;
        var type = currentElement.type;
        if (type === 'Line'){
          // var newX = ((shifted === true) ? startX : currentX);
          // var newY = ((controlled === true) ? startY : currentY);
          // object.points([startX, startY, newX, newY]);
          if (shifted === true){
          ...
          var height = ((shifted || controlled) ? Math.abs(width) : (currentY - startY));

          object.x( (width < 0) ? currentX : startX );
          object.y( (height < 0) ? currentY : startY );
          object.width(Math.abs(width));
          object.height(Math.abs(height));
        }

        if (type === 'Group'){
          variable_layer.add(object);
        } else {
    ...
    // ###################### DRAW FUNCTIONS #########################################################
    drawGroup: function drawGroup(event) {
      if (currentElement) {
        updateCurrentCoords(event);
  ...
  function selectedElement(){
    ...
    var group = elem.findAncestors('Group')[0];
    if (group) {
      // For finding I need the concept of a label, a label will have many shapes
      // and in that collection I can filter through them to get the one with the correct ID
      // find shape with same ID as group
    } else {
      // return shape with same ID as elem
    }
    // temporary: this should be a shape object not a Konva object
    return selected;
  }
  ...
  // #################### HOVER OVER ###############################################################
  // use event delegation to update pointer style and apply borders
  stage.on('mouseover', function(evt) {
    // #TODO: add getShape in here, its cleaner and safer to use
  ...
  function getShape(elem){
    // #TODO
  }

  function selectElementShape(elem){
    // Once we have getShape this method is not needed, it will simply be getShape(elem).select()
    var elemId = elem.getId();
    var group = elem.findAncestors('Group')[0];
    if (group) {
      // For finding I need the concept of a label, a label will have many shapes
  ...
  function shape(type){
    ...
    this.initialise = function(type){
      this.primary = drawFunctions[('init' + type)]();
      currentElement = this;
      ...
      div_paper.onmousemove = drawFunctions.drawShape;
    }
