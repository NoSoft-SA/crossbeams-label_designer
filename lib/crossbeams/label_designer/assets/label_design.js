  ...
  var Shape = function Shape(){
    ...
    return{
      ...
      initialise: function(type){
        this.primary = this[('init' + type)]();
        drawEnv.currentElement = this;

        this.type = type;
        if ('Group' === type){
          this.drawn = this.getRectangle(this.primary);
          this.textBox = this.getTextBox(this.primary);
        } else if ('Text' === type){
          this.textBox = this.primary;
        } else{
          this.drawn = this.primary;
        }
        // this.shapeId = ( (this.drawn != undefined) ? this.drawn._id() : this.textBox._id() );
        this.shapeId = ('Group' === type) ? this.drawn._id : this.primary._id;
      ...
      reinitialise: function(konva_shape){
        ...
        if (this.type === 'Group'){
          this.drawn = this.getRectangle(konva_shape);
          this.textBox = this.getTextBox(konva_shape);
      ...
      remove: function(){
        // #TODO
        // remove self
        // remove from drawEnv.stage
        // remove from layer
      },

      rotate: function(){
        var new_rotation = this.primary.rotation() + 90;
      ...
      getTextBox: function(group){
        var textbox = group.getChildren(function(node){
          return node.getClassName() === 'Text';
        });
        return textbox[0];
      },
      getRectangle: function(group){
        ...
        if (drawEnv.currentElement) {
          ...
          if (type === 'Line'){
            ...
            objects.forEach(function(obj){
              ...
              obj.x( (width < 0) ? drawEnv.currentX : drawEnv.startX );
              obj.y( (height < 0) ? drawEnv.currentY : drawEnv.startY );
              if (type === 'Ellipse'){
  ...
  var Label = function Label(){
    ...
    return {
      ...
      newShape: function(){
        ...
        myShape.initialise(drawEnv.drawing);
      ...
      getShapeByElem: function(elem){
        return this.getShapeById(elem._id);
      }
